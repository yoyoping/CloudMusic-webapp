...
export default {
  ...
  methods: {
    ...
    songInfo() {
      ...
      this.songDta.duration = isNaN(parseInt(this.$refs.audio.duration))
        ? 0
        : parseInt(this.$refs.audio.duration); // 当前歌曲总时长
      if (this.isLeave) {
        this.songDta.currentTime = parseInt(this.$refs.audio.currentTime); // 当前播放时间
      }
      // console.log(this.songDta.currentTime)
      this.songDta.timeProgress =
        (this.songDta.currentTime / this.songDta.duration) * 100; // 当前播放的时间占百分比
     ...
     */
    playSwitch(type) {
      let currentIndex = this._.findIndex(this.playList, {
        id: this.currentSongId
      }); // 获取当前音乐的在记录列表中的下标
      let redeyMusic;
      if (this.mode === 1 || this.mode === 3) {
        // 顺序循环播放(当单曲模式切换也是顺序播放，只有如果不切换会单曲循环)
        if (type === "prev") {
          // 上一首
          if (currentIndex === 0) {
